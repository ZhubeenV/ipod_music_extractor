...
def write_metadata_mp3(path, track, artwork_bytes, artwork_fmt):
    try:
        audio = ID3(path)
...
def write_metadata_m4a(path, track, artwork_bytes, artwork_fmt):
    try:
        audio = MP4(path)
    except Exception:
        return
...
def write_metadata_flac(path, track, artwork_bytes, artwork_fmt):
    try:
        audio = FLAC(path)
    except Exception:
        return
...
def copy_track(track, output_root: str) -> str:
    ...
    # Flat filename: "Artist - Album - Song.ext"
    filename  = f"{artist} - {album} - {name}{ext}"
    ...
    return f"[OK] {filename} ({meta_status})"
